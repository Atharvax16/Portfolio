/* ════════════════════════════════════════
   ...
   ════════════════════════════════════════ */
export const P = {
  paper: "#F4F0E0", paper2: "#FFFFFF", ink: "#16181D", sub: "#585C64",
  ...
  bg: "#F4F0E0", bgAlt: "#F1F0EA", text: "#16181D", muted: "#585C64",
...
export const TRACKS = [
  ...
  reproductions: [
    ...
    { name: "SSL objectives on medical images — controlled comparison", detail: "One encoder (params asserted identical), diffusion vs MAE vs JEPA, same budget and tap — probe 0.971 / 0.960 / 0.928 against an untrained encoder at 0.937, and the timestep curve reproduced", state: "run" },
...
export const READING_LOG = [
  ...
  },
  {
    ...
    hasNotebook: false,
    reproduced: "Rebuilt as an interactive sketch in §7 — the noise schedule on screen is computed, not quoted. Then ran the comparison the paper skipped: one encoder (params asserted identical), diffusion vs MAE vs JEPA, same budget, same tap, same probe, on PneumoniaMNIST over 3 seeds. Under the frozen probe the paper's direction holds — diffusion 0.971 vs MAE 0.960 test AUROC — but an untrained encoder scores 0.937, JEPA lands below it at 0.928, and fine-tuning closes the gap to 0.002. The rise-then-fall timestep curve reproduces, with the advantage over MAE gone by t ≈ 200. Walk it in §7.",
  },
  {
    paper: "CMANet: Cross-Modal Attention Network for 3-D Knee MRI and Report-Guided Osteoarthritis Assessment",
    authors: "Harari, Rajabzadeh-Oghaz, Hosseini et al.",
    year: "2025",
    area: "Medical imaging",
    takeaway: "Read for the clinical semantics before the architecture, and the semantics turned out to be the argument. Two kinds of label sit on the same knee. KL grade is Kellgren–Lawrence 0–4, an ordinal severity scale read off a plain radiograph from osteophytes and joint-space narrowing — one number for the whole joint, how far along it is. Voxel-level masks for cartilage defects, bone-marrow lesions and effusion say the other thing: where the damage actually is. The tension is sitting in the paper's own introduction — MRI shows cartilage thinning and BMLs long before either surfaces on a radiograph, and the target being predicted is still the radiograph's grade, so the more sensitive modality is being scored against the coarser, later scale. The labels that would say what an X-ray cannot are precisely the ones nobody has: 68 of 642 patients carry expert masks, because voxel annotation is a radiologist-hour per scan. That scarcity is what the report is doing here. The pitch is that a radiology report is not a weaker label but a different kind — it carries diagnostic semantics, which compartment, how severe, what the finding is called — so a phrase like “subchondral edema in the medial femoral condyle” is pulled out by a RadLex/SNOMED entity pipeline and aligned to an anatomical region, and region-level supervision arrives free inside prose that was written anyway. Interpretability then falls out of the same wiring instead of being bolted on afterwards: a prediction traces back to the report phrase and the anatomical region that carried it, which is a stronger claim than a saliency map, because the explanation lands in the vocabulary a radiologist already argues in. Same setting as the RSNA thread in §2 — knee MRI whose only scalable supervision is the prose written next to it — but one step further along: there the reports are mined offline into a static label set, here they stay inside the model. Which is the thing I want to hold against the architecture tomorrow. The report is an input at test time, not only at training, so the model grades OA severity while reading the radiologist's own interpretation of that same scan — and the ablation the paper leans on hardest (drop the text, mean Dice falls 0.729 → 0.672) is the shape a leakage result would also take. The discussion meanwhile proposes using it to produce a preliminary assessment before the official report is written, which that dependency rules out. Numbers to keep honest: n = 96 test set, a medRxiv preprint with no peer review, no external cohort. The architecture — the asymmetric cross-modal attention, and the alignment module doing the actual grounding — is tomorrow's read.",
    link: "https://doi.org/10.1101/2025.06.08.25329119",
    hasNotebook: false,
  },
];
...
export const ARCHITECTURES = [
  ...
  },
  {
    ...
    note: "denoise to learn anatomy · tap the bottleneck · what the skips carry · t as an abstraction dial",
    steps: "the pretext task → the tap → the skips → t as an abstraction dial → the MAE comparison → three planes, two fusions → what it never measures",
    intro: "Fifteen thousand knee MRIs with nothing written about them. The trick is to manufacture a label — add a known Gaussian, ask the network to name it — and the claim is that **to denoise well you have to learn anatomy**, because past a certain noise level the only way to guess the static is to guess the knee underneath it. That is the masked autoencoder's argument with one operator swapped: MAE deletes 75% of the patches, diffusion attenuates *everything* by a continuous amount. Two things in this sketch are worth the clicks. The first is that nothing is ever generated — a diagnosis comes from **one forward pass, read at the bottleneck**, with the decoder's output discarded unread, which is what keeps a hallucinated reconstruction out of the clinical path. The second is measured on the left of step 4: the paper's default tap sits at t = 30, where the schedule has removed almost nothing — *98.8% of the variance is still the scan*. The backbone is being queried where there is barely anything to denoise, and that is precisely why the features stay anchored to the image rather than to the prior. The last two steps are the honest ones: the fusion that scores highest is the one that cannot tell you which plane earned the call, and the MAE-versus-diffusion claim the whole paper leans on is **asserted, never run**.",
  },
  {
    key: "sslcompare", name: "One encoder, three objectives — the SSL comparison", short: "SSL comparison", family: "Self-supervised",
    status: "live", component: "SslCompareWalkthrough", year: 2026,
    note: "the baseline OrthoDiffusion asserted and never ran — measured",
    steps: "one trunk → one tap, one probe → the probe result → the timestep curve → fine-tuning closes it → what it settles",
    intro: "OrthoDiffusion argues that denoising beats masked reconstruction as a medical representation learner, and never runs an MAE baseline to check. This is that baseline, built small enough to finish on a laptop and controlled hard enough to mean something: **one** convolutional encoder — same class, same config, param counts asserted identical to the byte at 4,058,656 — with diffusion, MAE and JEPA hung off it, the same 4000-step budget each, read at the same bottleneck under the same frozen probe. Three results, all measured over three seeds on PneumoniaMNIST. Under the probe the paper's claim holds: **diffusion 0.971 against MAE's 0.960** test AUROC. The control nobody runs says how much to make of that — an *untrained* encoder probes at **0.937**, so the entire benefit of pretraining here is three points, and JEPA at 0.928 lands *below* it. And the ranking does not survive the paper's own protocol: fine-tune every arm and the gap collapses from eleven thousandths to two. Step 4 is the one I care about most — the rise-then-fall timestep curve reproduces on completely different anatomy, and it shows the advantage over MAE evaporating by t ≈ 200 and reaching *random-encoder* territory at t = 500.",
  },
  {
  ...
  {
    key: "abmil", name: "Attention-based MIL — learning which instance mattered", short: "Attention-MIL", family: "Medical imaging",
    status: "live", component: "AttentionMilWalkthrough", year: 2018,
    note: "one label per bag · a trainable pooling operator · localisation for free",
    steps: "the bag → decide late → the operator → the attention net → the rare positive → the free map → what it throws away",
